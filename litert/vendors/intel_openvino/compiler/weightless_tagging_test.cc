...
// Every weight constant (one whose name the bank knows) is tagged with an
// attribute whose offset and size match the bank; the returned count equals the
// number of tagged constants.
TEST(WeightlessTaggingTest, TagsWeightConstants) {
  ...
  for (const auto& node : ov_model->get_ops()) {
    ...
    if (offset.has_value()) {
      ...
      // Frontend-synthesized constants the bank never saw stay untouched.
