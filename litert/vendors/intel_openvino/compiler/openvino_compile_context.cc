...
::litert::Expected<OpenVinoCompileContext> OpenVinoCompileContext::Create(
  ...
  for (int i = 0; i < num_custom_options; ++i) {
    ...
    if (!key.empty()) {
      ...
      if (key == "sdpa_preserve_q_heads") {
        LITERT_LOG(LITERT_INFO, "Custom config: sdpa_preserve_q_heads = %s",
                   value.c_str());
        context.sdpa_preserve_q_heads_ = (value == "true");
        continue;
      }
...
void OpenVinoCompileContext::OptimizeModel(
  ...
  if (device_ == "NPU") {
    NpuOptimizer()
        ...
        .SetSdpaPreserveQHeads(sdpa_preserve_q_heads_)
        .Run(model);
  }
}
